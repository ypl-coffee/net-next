					   ...
					   const struct iphdr *iph, bool ingress)
{
	...
	if (ingress)
		sk = inet_lookup_established(net, &tcp_hashinfo, iph->saddr, th->source,
					     iph->daddr, ntohs(th->dest), dif);
	else
		sk = inet_lookup_established(net, &tcp_hashinfo, iph->daddr, th->dest,
					     iph->saddr, ntohs(th->source), dif);	/* inet_iif()? */
					   ...
					   const struct iphdr *iph, bool ingress)
{
	...
	if (ingress)
		sk = udp4_lib_demux_lookup(net, uh->dest, iph->daddr, uh->source, iph->saddr, dif);
	else
		sk = udp4_lib_demux_lookup(net, uh->source, iph->saddr, uh->dest, iph->daddr, dif);
...
static inline void ypl(struct sk_buff *skb, bool before)
{
	if (skb->sk)
		printk("skbedit: %s: inet_dport: %d\n",
		       before ? "before" : "after", ntohs(inet_sk(skb->sk)->inet_dport));
}
